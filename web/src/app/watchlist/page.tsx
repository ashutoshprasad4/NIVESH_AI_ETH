  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      ...
      await Promise.all(watchlist.map(async (stock) => {
        try {
          const res = await fetch(`/api/market?symbol=${stock.symbol}&period=5d&interval=1d`);
          const data = await res.json();
          if (!data.error) results[stock.symbol] = { currentPrice: data.currentPrice, changePercent: data.changePercent };
        } catch {}
      }));
      setLiveData(results);
    };
